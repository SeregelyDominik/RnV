...
def load_audio(path: Path, target_sr: int = 16000) -> torch.Tensor:
    """
    Load WAV or MP3, resample to 16 kHz, convert to mono.
...
def train_segmenter(feats_dir, checkpoint_path, gamma):
    ...
    for feats_path in tqdm(feats_paths):
        wavs_dir = feats_path.parent.parent / "wavs"
        stem = feats_path.stem

        wav_path = wavs_dir / f"{stem}.wav"
        mp3_path = wavs_dir / f"{stem}.mp3"
        ...
        if wav_path.exists():
            ...
            logger.warning("No audio found for %s", stem)
            continue
