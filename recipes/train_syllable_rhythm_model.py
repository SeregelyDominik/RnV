...
def get_speaker_peak_to_peak_and_silence_durations(syllable_segmenter, audio_filepaths):
    ...
    for audio_path in tqdm(audio_filepaths):
        ...
        p2p, sil = syllable_segmenter.get_audio_peak_to_peak_and_silence_durations(wav, feats)

        if len(p2p) == 0:
            print(f"No peaks detected in {audio_path}")
...
def compute_speaker_rhythm_model(speaker_id, audio_data_path, segmenter_checkpoint, output_dir):
    ...
    print("Num peak-to-peak durations:", len(speaker_peak_to_peak_durations_in_s))
    print("Sum peak-to-peak durations:", sum(speaker_peak_to_peak_durations_in_s))
    print("First few durations:", speaker_peak_to_peak_durations_in_s[:10])
